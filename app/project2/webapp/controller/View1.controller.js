sap.ui.define([
    ...
    "sap/ui/table/Table",
    "sap/ui/table/Column"
], (Controller, JSONModel, MessageToast, Table, Column) => {
    "use strict";
    ...
    return Controller.extend("project2.controller.View1", {
        ...
        _loadSitesFromBackend: function () {
            const oODataModel = this.getOwnerComponent().getModel("odata") || this.getOwnerComponent().getModel() || this.getOwnerComponent().getModel("ReportingModel");
            const sBase = oODataModel.sServiceUrl.replace(/\/?$/, "/");
            const sUrl = sBase + "T_PackagingSite";
            jQuery.ajax({
                url: sUrl,
                type: "GET",
                contentType: "application/json",
                ...
                error: (oErr) => {
                    sap.m.MessageBox.error("Failed to load T_PackagingSite: " + (oErr.responseText || oErr.statusText));
                }
            });
            const sUrl1 = sBase + "M_PackagingSite";
            jQuery.ajax({
                url: sUrl1,
                type: "GET",
                contentType: "application/json",
        ...
        onCreatedEmailByVH: function () {
            if (!this._oEmailByVH) {
                this._oEmailByVH = sap.ui.xmlfragment("project2.fragment.EmailIDActivity", this);
        ...
        onEmailActByVHSearch: function (oEvent) {
            const sQuery = (oEvent.getParameter("value") || oEvent.getParameter("newValue") || "").trim();
            const dlg = this._oEmailByVH;
            if (!dlg) return;
            const oBinding = dlg.getBinding("items");
            if (!oBinding) return;
            ...
            oBinding.filter([new Filter({ filters: [new Filter("USERID", FO.Contains, sQuery)], and: false })]);
        },
        ...
        onChangedByVH: function () {
            if (!this._oChangedByVH) {
                this._oChangedByVH = sap.ui.xmlfragment("project2.fragment.EmailID", this);
        ...
        onPackagingInput: function () {
            if (!this._oValueHelpDialogPack) {
                this._oValueHelpDialogPack = sap.ui.xmlfragment("project2.fragment.PackingSiteActivity", this);
        ...
        onProductInputClick: function () {
            if (!this._oValueHelpDialog) {
                this._oValueHelpDialog = sap.ui.xmlfragment("project2.fragment.PackingSiteCode", this);
...
this.byId("packagingSiteBox").setVisible(true);
 ...
 this.byId("UserInactiveSiteBox").setVisible(false);
        ...
        _exportPDF: function (aData, sTitle) {
            ...
            var sLogoPath = sap.ui.require.toUrl("project2/img/Viatrislogo.jpeg");

            var sBgPath = sap.ui.require.toUrl("project2/img/background_img_pdf.jpg");
      ...
      Created By: ${escHtml(createdBy || "-")}
        ...
        onSummaryFilter: function () {
            ...
            var aFiltered = aSource.filter(function (oItem) {
                ...
                ["batchNo", "compCode", "pkgSiteIDName", "createdBy", "comments", "modifiedBy"].forEach(function (sKey) {
        ...
        onSummaryFilterReset: function () {
            ...
            var oFilterRows = this.byId("filterRows");
            if (oFilterRows) oFilterRows.destroyItems();
